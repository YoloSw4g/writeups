...
msg = "BONJOUR JE SUIS SharifCTF ET je suis contest"
key = "OKHEY"
...
#	print(ctxt)		# KPDPDGYJXNUSOIGOJDUSUQGFSHJUGIEAXJZUQVDKSCMQKXIR
        med_msg = playfair_dec(key, ctxt)
#        print(med_msg)
        ctxt = "KPDPDGYJXNUSOIGOJDUSUQGFSHJUGIEAXJZUQVDKSCMQKXIR"
        alphabet = ascii_uppercase
        for perm in itertools.permutations(alphabet, r=5):
            med_key = ''.join(perm) + ascii_uppercase
            med_key = make_key(med_key)
            med_msg = playfair_dec(med_key, ctxt)
            if re.search("SHARIFCTF", med_msg) and re.search("CONTEST", med_msg):
                print med_msg
